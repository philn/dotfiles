...
if __name__ == '__main__':
    ...
    allowed_lib_names = opts.packages #get_library_names_from_packages(opts.packages, opts.src_dir, logfile) if opts.packages else []
    if opts.command == 'build':
        ...
        for f in glob.glob(str(target_dir / '*.so'), recursive=True):
            so_file = P(f)
            if so_file.stem[3:] not in allowed_lib_names:
                print(f'Skipping {so_file.name} (not in --packages)', file=logfile)
                continue
            print(f'Symlinking {so_file}', file=logfile)
            os.system(f"ln -f -s {so_file} {so_file}.0")
